\section*{Diario delle Modifiche}
\begin{longtabu} to \textwidth {
	X[4,l,p]
	X[4,l,p]
	X[4,l,p]
	X[8,l,p]}
	\toprule
		\rowcolor{blue!20} Versione & Data & Autore & Descrizione \\
		\midrule
		\endhead
		\arrayrulecolor{gray}
		3.0.0 & 23/08/2017 & Pezzuto Francesco & Approvazione Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		2.1.0 & 20/08/2017 & Sanna Giovanni & Verifica Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		2.0.1 & 12/08/2017 & Bonolo Marco & Corretti errori segnalati in RQ\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		2.0.0 & 02/07/2017 & Bonolo Marco & Approvazione Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		1.1.0 & 01/07/2017 & Sanna Giovanni & Verifica Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		1.0.2 & 28/06/2017 & Sovilla Matteo & Modifiche in seguito a correzione AdR\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		1.0.1 & 22/05/2017 & Pace Giulio & Apportate correzioni errori segnalati in RP\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		1.0.0 & 08/05/2017 & Bonato Enrico & Approvazione Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.1.0 & 07/05/2017 & Sanna Giovanni & Verifica Documento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.8 & 06/05/2017 & Pace Giulio & Stesura Sezione Tracciamento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.7 & 06/05/2017 & Sovilla Matteo & Stesura Sezione Tracciamento\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.6 & 05/05/2017 & Pace Giulio & Stesura sezione Componenti e Classi Principali\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.5 & 05/05/2017 & Bonolo Marco & Stesura sezione Componenti e Classi Principali\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.4 & 02/05/2017 & Pezzuto Francesco & Sistemata Introduzione; Aggiunte sezioni Tecnologie Utilizzate, Architettura generale, Design pattern utilizzati\\
		\addlinespace[0.2em]
		\midrule
		\addlinespace[0.2em]
		0.0.1 & 01/05/2017 & Pace Giulio & Creazione scheletro del documento e stesura della sezione Introduzione\\
		\arrayrulecolor{black}
		\addlinespace[0.4em]
		
	\bottomrule
\end{longtabu}
